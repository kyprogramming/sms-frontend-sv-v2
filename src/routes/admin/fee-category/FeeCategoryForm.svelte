<script lang="ts">
	...
	import { feeCategoryFormSchema, type FeeCategoryPayload } from '$lib/utils/schemas';
	...
	export function initializeFeeCategoryFormData(): FeeCategoryPayload {
		return { name: '', description: '', active: true };
	}

	// Reactive form state
	let formData: FeeCategoryPayload = $state(initializeFeeCategoryFormData());
	let touched: Partial<Record<keyof FeeCategoryPayload, boolean>> = $state({});
	...
	function handleChange(field: keyof FeeCategoryPayload, value: string | boolean): void {
