<script lang="ts">
    ...
    function updateMonthsInView(): void {
      if (endDate) {
        ...
        if (leftCalendar.month.format('YYYY-MM') !== startDate.format('YYYY-MM') && 
    ...
    function calculateChosenLabel(): void {
      for (const [label, range] of Object.entries(ranges)) {
        if (timePicker) {
          ...
          if (startDate.format('YYYY-MM-DD') === range[0].format('YYYY-MM-DD') && 
  ...
  <div class="daterangepicker" bind:this={element}>
    <input 
      ...
      value={startDate.format(locale.format) + (singleDatePicker ? '' : locale.separator + endDate.format(locale.format))}
      on:click={toggle}
      readonly
    />
        ...
        <div class="ranges">
          <ul>
            {#each Object.entries(ranges) as [label], i}
              <li 
                class:active={chosenLabel === label}
                on:click={() => selectRange(label)}
              >
                {label}
              </li>
            {/each}
            {#if showCustomRangeLabel}
              <li 
                class:active={chosenLabel === locale.customRangeLabel}
                on:click={() => chosenLabel = locale.customRangeLabel}
              >
